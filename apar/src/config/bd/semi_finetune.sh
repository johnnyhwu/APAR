cd ../..

python semi_finetune.py \
--exp_name tw_hp_txg_b-APAR-semi_finetune_division\ \(gczksfh5\) \
--batch_size 256 \
--val_ratio 0.1 \
--val_freq 100 \
--learning_rate 0.0005 \
--total_epoch 100 \
--device 0 \
--seed 47 \
--dataset tw_hp_txg_b \
--numerical_feature_encoding log2_delta \
--categorical_feature_encoding label \
--target_encoding log2_delta \
--pretrain_wandb_id gczksfh5 \
--pretrain_wandb_name ckpt/self_pretrain/tw_hp_txg_b-APAR-self_pretrain_division.pt \
--pred_loss_weight 1.0 \
--csty_loss_weight 0.01 \
--ftpy_loss_weight 0.01

python semi_finetune.py \
--exp_name tw_hp_txg_b-APAR-semi_finetune_division\ \(gczksfh5\) \
--batch_size 256 \
--val_ratio 0.1 \
--val_freq 100 \
--learning_rate 0.0005 \
--total_epoch 100 \
--device 0 \
--seed 47 \
--dataset tw_hp_txg_b \
--numerical_feature_encoding log2_delta \
--categorical_feature_encoding label \
--target_encoding log2_delta \
--pretrain_wandb_id gczksfh5 \
--pretrain_wandb_name ckpt/self_pretrain/tw_hp_txg_b-APAR-self_pretrain_division.pt \
--pred_loss_weight 1.0 \
--csty_loss_weight 0.1 \
--ftpy_loss_weight 0.01

python semi_finetune.py \
--exp_name tw_hp_txg_b-APAR-semi_finetune_division\ \(gczksfh5\) \
--batch_size 256 \
--val_ratio 0.1 \
--val_freq 100 \
--learning_rate 0.0005 \
--total_epoch 100 \
--device 0 \
--seed 47 \
--dataset tw_hp_txg_b \
--numerical_feature_encoding log2_delta \
--categorical_feature_encoding label \
--target_encoding log2_delta \
--pretrain_wandb_id gczksfh5 \
--pretrain_wandb_name ckpt/self_pretrain/tw_hp_txg_b-APAR-self_pretrain_division.pt \
--pred_loss_weight 1.0 \
--csty_loss_weight 0.2 \
--ftpy_loss_weight 0.01

python semi_finetune.py \
--exp_name tw_hp_txg_b-APAR-semi_finetune_division\ \(gczksfh5\) \
--batch_size 256 \
--val_ratio 0.1 \
--val_freq 100 \
--learning_rate 0.0005 \
--total_epoch 100 \
--device 0 \
--seed 47 \
--dataset tw_hp_txg_b \
--numerical_feature_encoding log2_delta \
--categorical_feature_encoding label \
--target_encoding log2_delta \
--pretrain_wandb_id gczksfh5 \
--pretrain_wandb_name ckpt/self_pretrain/tw_hp_txg_b-APAR-self_pretrain_division.pt \
--pred_loss_weight 1.0 \
--csty_loss_weight 0.3 \
--ftpy_loss_weight 0.01

python semi_finetune.py \
--exp_name tw_hp_txg_b-APAR-semi_finetune_division\ \(gczksfh5\) \
--batch_size 256 \
--val_ratio 0.1 \
--val_freq 100 \
--learning_rate 0.0005 \
--total_epoch 100 \
--device 0 \
--seed 47 \
--dataset tw_hp_txg_b \
--numerical_feature_encoding log2_delta \
--categorical_feature_encoding label \
--target_encoding log2_delta \
--pretrain_wandb_id gczksfh5 \
--pretrain_wandb_name ckpt/self_pretrain/tw_hp_txg_b-APAR-self_pretrain_division.pt \
--pred_loss_weight 1.0 \
--csty_loss_weight 0.4 \
--ftpy_loss_weight 0.01

python semi_finetune.py \
--exp_name tw_hp_txg_b-APAR-semi_finetune_division\ \(gczksfh5\) \
--batch_size 256 \
--val_ratio 0.1 \
--val_freq 100 \
--learning_rate 0.0005 \
--total_epoch 100 \
--device 0 \
--seed 47 \
--dataset tw_hp_txg_b \
--numerical_feature_encoding log2_delta \
--categorical_feature_encoding label \
--target_encoding log2_delta \
--pretrain_wandb_id gczksfh5 \
--pretrain_wandb_name ckpt/self_pretrain/tw_hp_txg_b-APAR-self_pretrain_division.pt \
--pred_loss_weight 1.0 \
--csty_loss_weight 0.5 \
--ftpy_loss_weight 0.01

python semi_finetune.py \
--exp_name tw_hp_txg_b-APAR-semi_finetune_division\ \(gczksfh5\) \
--batch_size 256 \
--val_ratio 0.1 \
--val_freq 100 \
--learning_rate 0.0005 \
--total_epoch 100 \
--device 0 \
--seed 47 \
--dataset tw_hp_txg_b \
--numerical_feature_encoding log2_delta \
--categorical_feature_encoding label \
--target_encoding log2_delta \
--pretrain_wandb_id gczksfh5 \
--pretrain_wandb_name ckpt/self_pretrain/tw_hp_txg_b-APAR-self_pretrain_division.pt \
--pred_loss_weight 1.0 \
--csty_loss_weight 0.6 \
--ftpy_loss_weight 0.01

python semi_finetune.py \
--exp_name tw_hp_txg_b-APAR-semi_finetune_division\ \(gczksfh5\) \
--batch_size 256 \
--val_ratio 0.1 \
--val_freq 100 \
--learning_rate 0.0005 \
--total_epoch 100 \
--device 0 \
--seed 47 \
--dataset tw_hp_txg_b \
--numerical_feature_encoding log2_delta \
--categorical_feature_encoding label \
--target_encoding log2_delta \
--pretrain_wandb_id gczksfh5 \
--pretrain_wandb_name ckpt/self_pretrain/tw_hp_txg_b-APAR-self_pretrain_division.pt \
--pred_loss_weight 1.0 \
--csty_loss_weight 0.7 \
--ftpy_loss_weight 0.01

python semi_finetune.py \
--exp_name tw_hp_txg_b-APAR-semi_finetune_division\ \(gczksfh5\) \
--batch_size 256 \
--val_ratio 0.1 \
--val_freq 100 \
--learning_rate 0.0005 \
--total_epoch 100 \
--device 0 \
--seed 47 \
--dataset tw_hp_txg_b \
--numerical_feature_encoding log2_delta \
--categorical_feature_encoding label \
--target_encoding log2_delta \
--pretrain_wandb_id gczksfh5 \
--pretrain_wandb_name ckpt/self_pretrain/tw_hp_txg_b-APAR-self_pretrain_division.pt \
--pred_loss_weight 1.0 \
--csty_loss_weight 0.8 \
--ftpy_loss_weight 0.01

python semi_finetune.py \
--exp_name tw_hp_txg_b-APAR-semi_finetune_division\ \(gczksfh5\) \
--batch_size 256 \
--val_ratio 0.1 \
--val_freq 100 \
--learning_rate 0.0005 \
--total_epoch 100 \
--device 0 \
--seed 47 \
--dataset tw_hp_txg_b \
--numerical_feature_encoding log2_delta \
--categorical_feature_encoding label \
--target_encoding log2_delta \
--pretrain_wandb_id gczksfh5 \
--pretrain_wandb_name ckpt/self_pretrain/tw_hp_txg_b-APAR-self_pretrain_division.pt \
--pred_loss_weight 1.0 \
--csty_loss_weight 0.9 \
--ftpy_loss_weight 0.01

python semi_finetune.py \
--exp_name tw_hp_txg_b-APAR-semi_finetune_division\ \(gczksfh5\) \
--batch_size 256 \
--val_ratio 0.1 \
--val_freq 100 \
--learning_rate 0.0005 \
--total_epoch 100 \
--device 0 \
--seed 47 \
--dataset tw_hp_txg_b \
--numerical_feature_encoding log2_delta \
--categorical_feature_encoding label \
--target_encoding log2_delta \
--pretrain_wandb_id gczksfh5 \
--pretrain_wandb_name ckpt/self_pretrain/tw_hp_txg_b-APAR-self_pretrain_division.pt \
--pred_loss_weight 1.0 \
--csty_loss_weight 1.0 \
--ftpy_loss_weight 0.01